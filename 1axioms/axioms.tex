...
\begin{description}
	...
	\item[Euclid of Alexandria] (c.\,300\,\BC) Collects and expands earlier work, especially that of the Pythagoreans. His compendium the \emph{Elements} motivates mathematical development in Eurasia and North Africa, remaining a standard school textbook well into the 1900s. The \emph{Elements} is an early exemplar of the axiomatic method at the heart of modern mathematics.
	
	\item[Later Greek Geometry] Archimedes' (c.\,270--212\,\BC) work on area and volume includes techniques similar to those of modern calculus. Apollonius studies conics (ellipses, parabolæ and hyperbolæ). Ptolemy's (c.\,\AD\,100--170) \emph{Almagest} applies basic geometry to astronomy and includes the foundations of trigonometry.
	
	\item[Post-Greek Geometry] Indian and Islamic mathematicians extend the work of Euclid and Ptolemy to develop early trigonometry (as well as algebra and our modern system of decimal enumeration).
...
\begin{center}
	\parbox{95pt}{\centering \includegraphics[alt={A circle is bisected by its diameter}]{thales-1}\\ Theorem 1}
	\qquad
	\parbox{70pt}{\centering \includegraphics[alt={the base angles of an isosceles triangle are equal}]{thales-2}\\ Theorem 2}
	\qquad
	\parbox{200pt}{\centering \includegraphics[alt={angle in a semicircle is right}]{thales-5}\\ Theorem 5}
...
% 	\end{tabular}
\end{center}
Arguments for Theorems 1 and 2 might be as simple as `fold.' Theorem 5 follows because the \textcolor{Green}{radius} of the circle splits the large triangle into two isosceles triangles; Theorem 2 says that these have equal base angles ($\alpha,\alpha$ and $\beta,\beta$), whence the apex $\alpha+\beta$ equals half the angle-sum in a triangle, namely a right-angle.
...
\begin{enumerate}
  \item By \emph{square,} the Greeks meant an honest square (shape)! To the ancient Greeks, Pythagoras was not a statement about multiplication (`squaring'): there is no algebra, no numerical length, and the equation $a^2+b^2=c^2$ won't be seen for another 2000 years.
...
% \]
\begin{quote}
	\tcbox[notitle,nophantom,colframe=red,colback=yellow!25!white]{Any theorem in an axiomatic system is true in any model of that system}
\end{quote}
...
\begin{example}{}{monoid}
	Here is the axiomatic system for a \emph{monoid}, built using the language of standard set theory (itself an axiomatic system).
 	...
 	\end{enumerate}
 	The integers under addition $(G,*)=(\Z,+)$ provide an abstract model; in this case the identity is $e=0$.\par
...
We unpack these ideas slightly, though our descriptions are necessarily vague: some notions must be clarified (e.g., \emph{valid proposition}) before these ideas can be made rigorous.
\begin{description}
	...
	\item[\normalfont\emph{Independence}] To demonstrate the independence of an axiom, exhibit two models: one in which all axioms are true, the other in which only the considered axiom is false. To naively demonstrate the independence of a consistent system of $n$ axioms requires $n+1$ models: one where each axiom is false. 
	\item[\normalfont\emph{Completeness}] This is very unlikely to hold for a useful axiomatic system, though \href{https://en.wikipedia.org/wiki/Complete_theory}{examples} exist. To standard way to demonstrate incompleteness is to exhibit an \emph{undecidable statement}: a valid proposition which can neither be proved nor disproved within the system. The undecidable statement (or its negations) can be viewed as a new independent axiom in an enlarged axiomatic system.\footnote{%
	...
	\begin{description}
		\item[\normalfont\emph{Consistent}] We have a (concrete) model.
 		...
 		\begin{itemize}
		  ...
		  \item The binary structure $\bigl(\{e,a,b\},*\bigr)$ defined by the following table satisfies A1 and A3, but not A2
		  \[%array causes trouble for SE tagging?
 		  	\begin{array}{c||ccc}
 		  		*&e&a&b\\\hline\hline
 		  		e&e&a&b\\
 		  		a&a&e&a\\
	  ...
	  \lstsp\lstsp\lstsp(A4)\lstsp Inverse:\quad $\forall g\in G,\ \exists g^{-1}\in G$ such that $g\ast g^{-1}=g^{-1}\ast g=e$.\smallbreak
		The new axiomatic system defined by the four axioms is also consistent and independent---this is the structure of a \emph{group.} Even this new system is incomplete; for instance, consider a new axiom of commutativity\ldots
	\end{description} 
\end{example*}
...
\begin{example}{Bus Routes}{}
	...
	\begin{enumerate}
	  ...
	  \begin{quote}%\def\arraystretch{1.2}
	  	%\tagpdfsetup{table/tagging=presentation}
		  %\begin{tabular}{@{}ll@{}}
		  	\makebox[50pt][l]{\emph{Stops}:}Downtown, \ Walmart, \ Ralph's, \ Main St., \ CVS, \ Trader Joe's, \ Zoo\par
		  	\makebox[50pt][l]{\emph{Route 1}:}Downtown, \ Walmart, \ Main St., \ CVS, \ Zoo, \ Downtown\par
		  	\makebox[50pt][l]{\emph{Route 2}:}Main St., \ CVS, \ Zoo, \ Ralph's, \ Downtown, \ Main St.\par
		  	\makebox[50pt][l]{\emph{Route 3}:}Walmart, \ Main St., \ Downtown, \ Ralph's, \ Main St., \ Walmart
		  %\end{tabular}
	  \end{quote}
