...
Analytic geometry arose in the early 1600s, thanks to the work of René Descartes and Pierre de Fermat. Their critical development was the introduction of an \emph{axis}\footnote{%
...
\begin{proof}
	The distance formula quickly shows that opposite sides have the same length:\par
	\begin{minipage}[t]{0.7\linewidth}\vspace{-6pt}
		\[
			\nm{BC}=\sqrt{x^2+y^2}=\nm{OA},\qquad
			\nm{AC}=\sqrt{v^2+w^2}=\nm{OB}
		\]
		Opposite sides are therefore congruent. SAS shows $\triangle OAC\cong \triangle CBO$. Euclid's discussion of alternate angles (pages \pageref{defn:parallel}--\pageref*{axiom:playfair}) forces opposite sides to be parallel.
	\end{minipage}
	\hfill
	  ...
	  \end{enumerate}
		...
		\end{enumerate} 
			...
			\begin{minipage}[t]{0.25\linewidth}\vspace{-10pt}
				\hfill\includegraphics[scale=0.95]{analytic-parabola}
			\end{minipage}
