	  ...
	  \item A triangle has angle of $\frac{2\pi}3$ radians between sides of lengths 2 and $\sqrt 3-1$. Find the length of the remaining side, and the remaining angles.
		...
		\begin{minipage}[t]{0.7\linewidth}\vspace{-4pt}
			\begin{enumerate}\itemsep1pt
				\item Use Pythagoras' to give an alternative proof of the sine rule (without the diameter conclusion).
				  
				\item Using the same approach, construct a proof of the cosine rule.
			 		
			 	\item Are your arguments valid if $D$ is not interior to $\cl{AC}$? Explain.
			\end{enumerate}
		\end{minipage}
		\hfill
...
At the heart of elementary geometry is \emph{congruence}, the idea that geometric figures can be essentially the same by without necessarily being equal. In analytic geometry, congruence is described algebraically using \emph{functions}. This is motivated by the fact that congruent segments have the same length.
...
\begin{enumerate}
	...
	\begin{minipage}[t]{0.31\linewidth}\vspace{0pt}
		\hfill\includegraphics[scale=0.95]{isom-scale}
	\end{minipage}

	\item $g(1,0)$ lies on the unit circle and therefore has the form\par
	...
	\begin{minipage}[t]{0.34\linewidth}\vspace{0pt}
		\hfill\includegraphics[scale=0.95]{isom-rot2}
	\end{minipage}
\end{enumerate}
...
\begin{examples}{}{isomexs}
	...
	\begin{enumerate}
		\item\label{ex:isomexs2}	We revisit Example \ref{ex:isomsetup} in matrix format:
	\[
		...
		\twovec xy+\twovec 31
	\]
	Since $\frac{\sin\theta}{\cos\theta}=\frac{4/5}{3/5}=\frac 43$, we see that its effect is to \emph{reflect} across the line through the origin making angle $\frac 12\tan^{-1}\frac 43\approx \ang{26.6}$ with the positive $x$-axis, before \emph{translating} by $(3,1)$. $\triangle_a$ has vertices $(0,0),(1,0),(2,-1)$ and is congruent to $\triangle_b$, two of whose vertices are $(1,2)$ and $(1,3)$. Find all isometries transforming $\triangle_a$ to $\triangle_b$ and the location(s) of the third vertex of $\triangle_b$.\smallbreak
	...
	\begin{minipage}[t]{0.76\linewidth}\vspace{-10pt}
	  ...
	  \begin{minipage}[t]{0.23\linewidth}\vspace{-10pt}
	  	\hfill\includegraphics[scale=0.95]{isom-ex}
	  \end{minipage}
	\end{enumerate}
\end{examples}
...
\begin{exercises}
	...
	\begin{enumerate}
	  ...
	  \begin{enumerate}
		...
		%\item Think through the list of Birkhoff's axioms and recall our earlier discussions of Euclidean geometry. Where do you think the uniqueness of parallels.
	\end{enumerate}
\end{exercises}
