...
\begin{exercises}
	...
	\]
	  ...
	  \end{enumerate}
		...
		\item A \emph{parabola} is a curve whose points are equidistant from a fixed point $F$ (the \emph{focus}) and a fixed line $\ell$ (the \emph{directrix}).\par
		  \begin{minipage}[t]{0.68\linewidth}\vspace{-4pt}
		\begin{enumerate}
			   \item Choose axes as shown in the picture so that $F=(0,a)$ and $\ell$ has equation $y=-a$. Find the equation of the parabola.
			   \item Now let $e$ be a positive constant ($\neq 0,1$). What is the equation of the parabola if $\ell$ has equation $y=-\frac ae$ and $\frac{\nm{PF}}{\nm{F\ell}}=e$? What happens in the limit $e\to 0^+$?
		\end{enumerate} 
			\end{minipage}
			\hfill
			\begin{minipage}[t]{0.31\linewidth}\vspace{-10pt}
				\hfill\includegraphics[alt={the geometric definition of a parabola}]{analytic-parabola}
			\end{minipage}
