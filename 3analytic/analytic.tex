...
Analytic geometry was conceived as a computational toolkit built on top of Euclid. At first, mathematicians felt the need to justify analytic arguments synthetically lest no-one believe their work.\footnote{This attitude persisted for some time; when Issac Newton published his groundbreaking \emph{Principia} in 1687, his presentation was largely synthetic, even though he had used co-ordinates in his derivations.} Synthetic geometry is not without its benefits, but its study has increasingly become a fringe activity; co-ordinates are just too useful to ignore.\smallbreak
...
\footnotetext{Some versions of analytic geometry permit negative angles or measures $\ge 2\pi$ radians. Note the distinction from synthetic geometry where all angles are smaller than a straight edge and $\angle BAC\cong\angle CAB$. In the picture, $\measuredangle CAB$ is \emph{not} the radian-measure of $\angle CAB$!}


Being ratios of lengths, radians are naturally \emph{unitless}; since arc-length scales with radius, the definition is independent of the radius of the circular arc. Indeed\par
...
\begin{description}
\item[\normalfont\emph{Sine Rule}] If $d$ is the diameter of the circumcircle (Definition \ref{defn:circledef}), then \ \smash{$\displaystyle\frac{\sin\alpha}a=\frac{\sin\beta}b=\frac{\sin\gamma}c=\frac 1d$}
...
In analytic geometry, congruence may be described \emph{algebraically.} Recall that two segments have the same length if and only if they are congruent; this motivates a definition.
...
\footnotetext{In the Greek, the prefix \emph{iso} means `same' and \emph{metros} `measure,' i.e.{} \emph{same length.}}
...
%\begin{enumerate}
  ...
  %\item $f\bigl((1-t)P+tQ\bigr)=(1-t)f(P)+tf(Q)$ \hfill ($f$ maps $\lin{PQ}$ bijectively to $\lin{f(P)f(Q)}$)
%\end{enumerate}
\end{lemm}
...
Our next task is to verify our intuition that isometries are rotations, reflections and translations. Given an isometry $f$, define $g$ via $g(X)=f(X)-f(O)$, where $O$ is the origin. Then $g$ is an isometry
...
\boldinline{Calculating with isometries}

Computation benefits from column-vector notation and matrix multiplication. If we write $\vx=\stwovec xy =\stwovec{r\cos\theta}{r\sin\theta}$ for the position vector of $X$ and apply the multiple-angle formulæ (page \pageref{sec:multangle}), then the expression for rotation becomes
\begin{align*}
...
%   The isometry which reflects across the line making angle $\ang{60}$ with the positive $x$-axis before translating 3 to the right and 2 down has the formula
...
% \begin{proof}
% \begin{enumerate}
%   \item This can be taken as a definition. To see that it is a reasonable definition requires either power series or elementary differential equations, topics best described elsewhere.
% 	\item Apply the multiple-angle formulæ (page \pageref{sec:multangle}):
% 	\begin{align*}
% 	e^{i\theta}e^{i\phi}&=(\cos\theta+i\sin\theta)(\cos\phi+i\sin\phi)\\
% 	&=\cos\theta\cos\phi-\sin\theta\sin\phi +i\sin\theta\cos\phi+i\cos\theta\sin\phi\\
% 	&=\cos(\theta+\phi)+i\sin(\theta+\phi)\\
% 	&=	e^{i(\theta+\phi)}
% 	\end{align*}
% 	The last part Performing an induction with $\theta=\phi$ and combining with $e^{i\theta}e^{-i\theta}=1$ gives the final result.\qedhere
% \end{enumerate}
% \end{proof}\goodbreak
...
%\boldsubsubsection{Relation to complex numbers}
...
\boldsubsubsection{Analytic Geometry as a \emph{Model}}
