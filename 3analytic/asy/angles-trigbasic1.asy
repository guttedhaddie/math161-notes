...
pair A=dir(60);
pair B=dir(180-60);
fill(O--arc(O,0.15X,0.15B)--cycle,Magenta);
draw(arc(O,X,dir(180)));
draw("$\theta$",arc(O,0.18X,0.18A));
draw(Label("$\pi-\theta$",Relative(0.75),align=4N),arc(O,0.15X,0.15B),Magenta);
draw("$\theta$",arc(O,0.18B,0.18dir(180)));

draw(-X--O--X);

draw("$\cos\theta$",O--(A.x,0),red+linewidth(1));
draw(Label("$\sin\theta$",Relative(0.4)),(A.x,0)--A,blue+linewidth(1));
draw((B.x,0)--O,red+linewidth(1));
draw((B.x,0)--B,blue+linewidth(1));
draw("$1$",O--A,lGreen+linewidth(1));
draw(O--B,lGreen+linewidth(1));
