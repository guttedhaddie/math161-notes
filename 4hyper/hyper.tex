...
\begin{proof}
	Given a rectangle $\square ABCD$, let $P\in\cl{CD}$ and drop the perpendicular to $R\in\cl{AB}$.\smallbreak
	$\square PRBC$ is a rectangle; if not, then one of $\square ARPD$ or $\square PRBC$ would have angle sum exceeding \ang{360}, thus contradicting Saccheri--Legendre.\smallbreak
	By Exercise \ref*{sec:hyp1}.\ref{exs:rectanglesplit}, $\ray{BP}$ splits $\square PRBC$ so that the \textcolor{orange}{marked angles} in the picture are congruent. In particular, $\ray{BP}$ crosses $\cl{CD}$ at the \emph{same angle} as it leaves $B$.\smallbreak
	...
	By SAA, $\triangle P_1PQ_1\cong\triangle PBR$, whence we obtain a second rectangle congruent to $\square PRBC$. Repeating the argument, we see that the sequence of points $P,Q_1,Q_2,Q_3,\ldots$ is also \emph{equidistant.} Since $\cl{CD}$ is \emph{finite,} this sequence must eventually\footnotemark{} pass $D$; we conclude that $\ray{BP}$ intersects $\lin{AD}$.
	\begin{center}
...
\boldsubsubsection{Interpretation of Isometries (non-examinable)}
...
\]
Applying trigonometric identities such as $\sec^2B=1+\tan^2B$, we finally conclude:
...
Armed with these results, one can solve any hyperbolic triangle numerically given the information in one of the triangle congruence theorems. Admittedly general ASA, SAA and AAA data are very messy to compute with, but others are straightforward.
...
\begin{exercises}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  Argue that the functions $f$ defined in Theorem \ref{thm:hypisomclass} really do map the interior of the unit disk to itself.
