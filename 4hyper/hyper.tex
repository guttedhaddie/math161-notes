...
Armed with these results, one can solve any hyperbolic triangle numerically given the information in one of the triangle congruence theorems. SAS and SSS are straightforward, though SAA is awkward, and general ASA and AAA data are very messy without some extra work (see Exercise \ref{exs:hcosruleII}).\footnote{Unlike in Euclidean geometry, knowing two angles doesn't automatically give you the third!}
...
\begin{exercises}
	...
	\begin{enumerate}\setcounter{enumi}{1}
	  ...
	  \item A hyperbolic right-triangle has non-hypotenuse sides $a=\cosh^{-1}2$ and $b=\cosh^{-1}3$. Find the hypotenuse, the angles and the area of the triangle.
		...
		\]
		\begin{enumerate}
		  \item Explain how this formula can be used to solve triangles given AAA or ASA data.
		  \item Prove the special case when $C=\frac\pi 2$ (see the trick in Example \ref*{ex:hyptrig}.\ref{ex:rtangleaaa}).\par
